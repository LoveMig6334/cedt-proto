...
const productionLines = [
  { id: 'L-01', name: 'สายการผลิต A — สันใน', status: 'กำลังผลิต', progress: 78, target: '180 กก.', current: '140 กก.', waste: '4.2%', statusCls: 'bg-[#D1FAE5] text-[#059669]' },
  { id: 'L-02', name: 'สายการผลิต B — สันนอก', status: 'หยุดพัก', progress: 45, target: '120 กก.', current: '54 กก.', waste: '3.8%', statusCls: 'bg-[#FEF3C7] text-[#D97706]' },
  { id: 'L-03', name: 'สายการผลิต C — เนื้อสับ', status: 'กำลังผลิต', progress: 92, target: '200 กก.', current: '184 กก.', waste: '5.1%', statusCls: 'bg-[#D1FAE5] text-[#059669]' },
];
      ...
      <div className="grid grid-cols-[1fr_280px] gap-[18px]">
        ...
        <div className="space-y-4">
          {productionLines.map((line) => (
            <div key={line.id} className="bg-white rounded-rlg border border-p-100 p-5 shadow-[0_1px_3px_rgba(0,0,0,.04)]">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="text-[13.5px] font-bold text-n-900 mb-0.5">{line.name}</div>
                  <div className="text-[11px] text-n-400">{line.id}</div>
                </div>
                <span className={`px-[9px] py-[3px] rounded-full text-[10.5px] font-semibold ${line.statusCls}`}>
                  {line.status}
                </span>
              </div>
              <div className="h-[6px] bg-n-100 rounded-full overflow-hidden mb-2">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-p-300 to-p-500"
                  style={{ width: `${line.progress}%` }}
                />
              </div>
              <div className="flex items-center justify-between text-[11px] text-n-500">
                <span>ผลผลิต: <b className="text-n-800">{line.current}</b> / {line.target}</span>
                <span>ของเสีย: <b className="text-fp-red">{line.waste}</b></span>
                <span className="text-p-500 font-semibold">{line.progress}%</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-rlg border border-p-100 shadow-[0_1px_3px_rgba(0,0,0,.04)]">
          <div className="px-5 pt-4 pb-[13px] border-b border-n-100">
            <div className="text-[13.5px] font-bold text-n-900">📦 วัตถุดิบคงเหลือ</div>
            <div className="text-[11px] text-n-500 mt-0.5">พร้อมใช้งานวันนี้</div>
          </div>
          <div className="p-4 space-y-4">
            {materialStock.map((m) => (
              <div key={m.name}>
                <div className="flex justify-between items-baseline mb-1">
                  <span className="text-[12.5px] font-semibold text-n-800">{m.name}</span>
                ...
                <div className="h-[6px] bg-n-100 rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-gradient-to-r from-[#34D399] to-fp-green" style={{ width: `${m.pct}%` }} />
                </div>
              </div>
            ))}
