...
function loadSavedOrders(): SourcingOrder[] {
  ...
  try {
    return JSON.parse(savedOrders) as SourcingOrder[];
  } catch (error) {
...
export function SourcingClient() {
  ...
  const [orders, setOrders] = useState<SourcingOrder[]>(loadSavedOrders);
  const [selectedOrder, setSelectedOrder] = useState<SourcingOrder | null>(
    null,
  );
  ...
  const isFormValid = () => {
    ...
    const hasSelectedItems = Object.values(orderItems).some(
      (item) => item.checked && parseFloat(item.quantity) > 0,
    );
    const hasOverLimitItems = Object.values(orderItems).some(
      (item) => item.checked && parseFloat(item.quantity) > 1000,
    );
  ...
  const handleMoveToIntake = (orderId: string) => {
    ...
    if (selectedOrder?.id === orderId) {
      setSelectedOrder((prev) =>
        prev ? { ...prev, status: "In Intake" } : null,
      );
    }
  };
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      ...
      const newSuppliersList = genericSuppliers
        .map((sup) => {
  ...
  return (
    <div>
      <div className="flex justify-between items-start mb-5.5">
        ...
        <div className="flex gap-2 p-1 bg-n-100 rounded-xl">
          <button
      ...
      {activeTab === "find" ? (
        <div className="grid grid-cols-[1fr_320px] gap-4.5">
              ...
              {suppliers
                .filter(
                  (sup) =>
                    ...
                    sup.meatTypes.some((m) =>
                      m.toLowerCase().includes(subSearchQuery.toLowerCase()),
                    ),
                )
                .map((sup) => (
                  <AnimatedCard
                    ...
                    className={`bg-white border-[1.5px] rounded-xl p-3.75 cursor-pointer transition-all hover:border-p-400 ${selectedSupplier?.name === sup.name ? "border-p-500 bg-p-50 shadow-[0_0_0_2px_rgba(236,72,153,0.1)]" : "border-p-100"}`}
                    hoverY={-2}
                  ...
                  >
                    ...
                    <div className="text-[11.5px] text-n-500 mb-2">
                      {sup.sub}
                    </div>
          ...
          {selectedSupplier ? (
            <div className="bg-white rounded-[16px] border border-p-100 shadow-[0_4px_20px_rgba(244,114,182,0.08)] flex flex-col h-150 overflow-hidden">
              <div className="px-5 pt-4 pb-3.25 border-b border-n-100 bg-linear-to-r from-white to-p-50 flex items-center justify-between">
                <div className="text-[14.5px] font-extrabold text-n-900 flex items-center gap-2">
                  {isCreatingPO
                    ? "📝 สร้างใบสั่งซื้อ (Generate PO)"
                    : "📋 รายละเอียดผู้จัดส่ง"}
                </div>
                <button
                  onClick={handleClosePanel}
                  className="text-[12px] font-semibold text-p-500 hover:text-p-600 cursor-pointer"
                >
                  ปิด (Close)
                </button>
              </div>
              ...
              {isCreatingPO ? (
                isSubmitted ? (
                  <div className="flex-1 flex flex-col p-6 overflow-y-auto scrollbar-thin scrollbar-thumb-p-200 scrollbar-track-transparent">
                    <div className="flex flex-col items-center text-center mb-6">
                      ...
                      <div className="text-[18px] font-extrabold text-n-900 mb-1">
                        ส่งใบสั่งซื้อสำเร็จ!
                      </div>
                      <div className="text-[12px] text-n-500">
                        ระบบได้ทำการส่งข้อมูลไปยังผู้จัดส่งแล้ว
                      </div>
                    </div>

                    <div className="bg-n-50 border border-n-100 rounded-xl p-5 font-mono text-[12px] text-n-800 space-y-4 shadow-sm">
                      <div className="flex justify-between border-b border-n-200 pb-2 mb-2">
                        <span className="font-bold text-n-400">SUBJECT:</span>
                        <span className="font-bold text-p-600 truncate">
                          {submittedOrderId} - FreshPro
                        </span>
                      </div>

                      <div>
                        <span className="block font-bold text-n-400 mb-1 uppercase tracking-wider text-[10px]">
                          TO (ถึง):
                        </span>
                        <div className="font-bold">{selectedSupplier.name}</div>
                        <div className="text-n-500 text-[11px]">
                      ...
                      <div>
                        ...
                        <div className="font-bold text-n-900">
                          {deliveryDate}
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div>
                          <label className="block text-[12px] font-bold text-n-700 mb-1.5">
                            วันส่งมอบ (Expected Delivery Date){" "}
                        ...
                        <div>
                          ...
                          <div className="space-y-2 border-[1.5px] border-n-200 rounded-[10px] p-3 bg-white">
                            {selectedSupplier.meatTypes.map((meat, idx) => (
                              <label
                                key={idx}
                                className="flex items-center gap-3 p-2 hover:bg-n-50 rounded-md cursor-pointer transition-colors"
                              >
                                <input
                                  type="checkbox"
                                ...
                                />
                                <span className="text-[13px] font-medium text-n-800 flex-1">
                                  {meat}
                                </span>
                                <input
                                  type="number"
                                  ...
                                  max={1000}
                                  onChange={(e) =>
                                    handleQuantityChange(meat, e.target.value)
                                  }
                                  className={`w-25 px-2 py-1 border-[1.5px] rounded-md text-[12px] outline-none focus:border-p-400 bg-white disabled:bg-n-100 ${parseFloat(orderItems[meat]?.quantity || "0") > 1000 ? "border-red-500 text-red-500" : "border-n-200"}`}
                ...
                <>
                  <div className="flex-1 p-6 overflow-y-auto scrollbar-thin scrollbar-thumb-p-200 scrollbar-track-transparent">
                    ...
                    <div className="text-[13px] text-n-500 mb-5">
                      {selectedSupplier.sub}
                    </div>

                    <div className="space-y-4">
                      <div className="p-4 rounded-xl bg-n-50 border border-n-100 transition-all hover:border-n-200">
                        <div className="text-[11.5px] font-bold text-n-400 uppercase tracking-widest mb-2">
                      ...
                      <div className="p-4 rounded-xl bg-n-50 border border-n-100 transition-all hover:border-n-200">
                        <div className="text-[11.5px] font-bold text-n-400 uppercase tracking-widest mb-3">
                          ประเภทเนื้อที่จำหน่าย (Meat Types)
                        </div>
                        <div className="flex flex-wrap gap-2">
                          {selectedSupplier.meatTypes.map((meat, idx) => (
                            <span
                              key={idx}
                              className="px-3 py-1.5 bg-white text-n-700 rounded-md text-[12px] font-bold border border-n-200 shadow-sm"
                            >
                              {meat}
                            </span>
                          ))}
                      ...
                      <div className="p-4 rounded-xl bg-p-50 border border-p-100 flex justify-between items-center transition-all hover:border-p-200">
                        <div className="text-[11.5px] font-bold text-p-500 uppercase tracking-widest">
                          ราคาเฉลี่ย (Price)
                        </div>
            ...
            <div className="bg-white rounded-[16px] border border-p-100 shadow-[0_4px_20px_rgba(244,114,182,0.08)] flex flex-col h-150 overflow-hidden">
              <div className="px-5 pt-4 pb-3.25 border-b border-n-100 bg-linear-to-r from-white to-p-50">
                <div className="text-[14.5px] font-extrabold text-n-900 flex items-center gap-2">
              ...
              <div className="flex-1 p-4 flex flex-col gap-3 overflow-y-auto scrollbar-thin scrollbar-thumb-p-200 scrollbar-track-transparent">
                {chatMessages.map((msg, i) => (
                  ...
                  >
                    ...
                    <div
                      className={`max-w-[80%] px-3.5 py-2.5 rounded-xl text-[13px] leading-relaxed shadow-sm ${msg.role === "ai" ? "bg-white border border-p-100 text-n-800 rounded-tl-none" : "bg-n-900 text-white rounded-tr-none"}`}
                    >
                      {msg.text}
                    </div>
                  </div>
                ...
                {isAiTyping && (
                  <div className="flex gap-2 items-start">
                    ...
                    <div className="max-w-[80%] px-4 py-3 rounded-xl text-[13px] bg-white border border-p-100 rounded-tl-none shadow-sm flex items-center gap-1">
              ...
              <div className="px-3 py-3 border-t border-n-100 bg-white flex gap-2 items-center">
                ...
                <button
                  ...
                  className="w-10 h-10 bg-linear-to-br from-p-400 to-p-500 hover:from-p-500 hover:to-p-600 disabled:from-n-300 disabled:to-n-400 border-none rounded-full cursor-pointer text-white text-[16px] shrink-0 items-center justify-center shadow-md transition-all active:scale-95 flex"
                >
                  <svg
                    width="18"
                    height="18"
        ...
        <div className="grid grid-cols-[1fr_360px] gap-4.5">
          ...
          <div className="space-y-3">
            ...
            {orders.length === 0 ? (
              <div className="bg-white border-[1.5px] border-n-100 rounded-[16px] p-10 text-center">
                ...
                <div className="text-n-900 font-bold mb-1">
                  ยังไม่มีประวัติการสั่งซื้อ
                </div>
                <div className="text-n-400 text-[13px]">
                  เริ่มต้นค้นหาผู้ส่งและสร้างใบสั่งซื้อรายการแรกของคุณ
                </div>
              </div>
            ) : (
              orders.map((order) => (
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-[12px] text-n-500 font-medium">
                          {order.supplier.name}
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div
                        className={`px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider mb-1.5 ${
          ...
          {/* Detail / Invoice View */}
          <div className="bg-white rounded-[20px] border-[1.5px] border-n-100 shadow-xl overflow-hidden flex flex-col h-150">
            {selectedOrder ? (
              <div className="flex-1 flex flex-col h-full">
                <div className="px-6 py-5 border-b border-n-100 bg-linear-to-r from-n-50 to-white flex justify-between items-center">
                  <div>
                    <div className="text-[13px] font-extrabold text-n-900">
                  ...
                  <div className="bg-white p-2 rounded-lg text-lg shadow-sm">
                    🐄
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-p-200">
                  <div className="mb-6">
                    <div className="text-[11px] font-bold text-n-400 uppercase mb-3 tracking-widest">
                      ข้อมูลผู้จัดส่ง
                    </div>
                    ...
                    <div className="space-y-2">
                      {Object.entries(selectedOrder.items)
                        ...
                        .map(([name, item]) => (
                          <div
                            key={name}
                            className="flex justify-between items-center py-2 px-3 bg-white border border-n-100 rounded-lg shadow-sm"
                          >
                            <span className="text-[12.5px] text-n-700 font-medium">
                              {name}
                            </span>
                            <span className="font-bold text-n-900 text-[13px]">
                              {item.quantity} กก.
                            </span>
                          </div>
                        ))}
                  ...
                  <div className="mb-6">
                    <div className="flex justify-between mb-3">
                      <div className="text-[11px] font-bold text-n-400 uppercase tracking-widest">
                        ใบแจ้งหนี้ (Mock Invoice)
                      </div>
                      <div className="text-[11px] font-bold text-p-500">
                        READY TO DOWNLOAD
                      </div>
                    </div>
                    <div className="bg-p-50/30 border-2 border-dashed border-p-200 rounded-2xl p-6 text-center group cursor-pointer hover:bg-p-50 transition-all">
                      ...
                      <div className="text-[13px] font-bold text-p-600 mb-0.5 underline">
                        View_Invoice_{selectedOrder.id}.pdf
                      </div>
                      <div className="text-[11px] text-n-400 font-medium">
                        คลิกเพื่อดูรายละเอียดใบแจ้งหนี้
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-10 text-center opacity-40">
                ...
                <div className="text-n-400 text-[12px]">
                  เพื่อดูรายละเอียดและดำเนินการต่อ
                </div>
              </div>
            )}
          </div>
