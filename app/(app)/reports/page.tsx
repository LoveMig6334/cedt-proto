    ...
    >
      ...
      <div className="flex justify-between items-end mb-7">
        <div>
          ...
          <p className="text-[13.5px] text-n-500">
            วิเคราะห์ข้อมูลประสิทธิภาพของโรงงานแบบเจาะลึก
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4.5 mb-6">
        {data.kpis.map((kpi) => (
          ...
          >
            <motion.div
              className={`absolute -top-3 -right-3 w-16 h-16 bg-linear-to-br ${kpi.up ? "from-p-50 to-p-100" : "from-n-50 to-n-100"}`}
              variants={{ hover: { scale: 1.25 } }}
              transition={{ type: "spring", stiffness: 300, damping: 25 }}
            />
            <div className="relative z-10">
              <div className="text-[12px] font-bold text-n-400 mb-1 uppercase tracking-wider">
                {kpi.label}
              </div>
              <div className="text-[28px] font-extrabold text-n-900 mb-0.5 tracking-tight">
                {kpi.value}
              </div>
      ...
      <div className="grid grid-cols-3 gap-5.5">
        ...
        <div className="col-span-2 bg-white border border-p-100 rounded-rlg p-6 shadow-sm">
          <div className="flex justify-between items-center mb-10">
            <div>
              <h3 className="text-[17px] font-extrabold text-n-900">
                {data.title}
              </h3>
              <p className="text-[12px] text-n-500">
                ปริมาณการผลิตเนื้อสะสม (กก.)
              </p>
            </div>
          ...
          <div className="flex items-end gap-3 h-60 px-2 mb-4">
            {data.chartData.map((d, i) => (
              ...
              >
                ...
                <div
                  className="w-full bg-n-50 rounded-t-md relative overflow-hidden transition-all duration-700 ease-out group-hover:brightness-95"
                  style={{ height: "90%" }}
                >
                  <div
                    className="absolute bottom-0 left-0 w-full bg-linear-to-t from-p-500 to-p-300 rounded-t-md transition-all duration-1000 ease-out"
                    style={{ height: `${d.h}%` }}
                  />
                </div>
                <div className="mt-4 text-[11px] font-bold text-n-400 group-hover:text-p-500 transition-colors uppercase">
                  {d.label}
                </div>
              </div>
            ))}
        ...
        <div className="flex flex-col gap-5.5">
          ...
          <div className="bg-white border border-p-100 rounded-rlg p-6 shadow-sm flex-1">
            <h3 className="text-[16px] font-extrabold text-n-900 mb-5">
              สัดส่วนตามประเภท
            </h3>
              ...
              {data.categories.map((cat) => (
                <div key={cat.name}>
                  <div className="flex justify-between items-end mb-2">
                    <div>
                      <div className="text-[13px] font-bold text-n-800">
                        {cat.name}
                      </div>
                      <div className="text-[11px] text-n-400">{cat.value}</div>
                    </div>
                    <div className="text-[14px] font-extrabold text-p-500">
                      {cat.pct}%
                    </div>
                  </div>
            ...
            <div className="mt-8 pt-6 border-t border-n-100">
              <div className="flex items-center gap-3 p-4 bg-cream rounded-r border border-p-50">
                ...
                <div>
                  <div className="text-[12px] font-bold text-n-800">
                    สรุปความเห็น AI
                  </div>
                  <div className="text-[10.5px] text-n-500 leading-tight">
                    สัปดาห์นี้สันนอกขายดีกว่าปกติ 15% แนะนำให้เพิ่มการผลิตสายนี้
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="mt-6 bg-white border border-p-100 rounded-rlg overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-n-100 flex justify-between items-center">
          <h3 className="text-[15px] font-extrabold text-n-900">
            รายการล่าสุดในรอบ
            {period === "week"
              ? "สัปดาห์"
              : period === "month"
          ...
          <button className="text-p-500 text-[12px] font-bold hover:underline">
            ดูทั้งหมด
          </button>
        </div>
        <div className="p-0">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-n-100">
              {[
                ...
                <tr
                  key={i}
                  className="hover:bg-cream transition-colors group cursor-pointer"
                >
                  ...
                  <td className="px-6 py-4 text-[12.5px] text-n-700">
                    {row.type}
                  </td>
                  <td className="px-6 py-4 text-[12.5px] text-n-700 font-medium">
                    {row.qty}
                  </td>
                  <td className="px-6 py-4 text-[13px] font-extrabold text-n-900">
                    {row.val}
                  </td>
                  <td className="px-6 py-4">
                    <span
                      className={`text-[11px] font-bold px-2.5 py-1 rounded-full bg-n-100 ${row.cls}`}
                    >
                      {row.qc}
                    </span>
                  </td>
